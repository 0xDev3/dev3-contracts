...
contract KNN3ProfileClient is ChainlinkClient, ConfirmedOwner {
    ...
    function requestBasicProfile(address _oracle, string memory _jobId)
        public
        onlyOwner
    {
    ...
    function requestSocialStatus(address _oracle, string memory _jobId)
        public
        onlyOwner
    {
        Chainlink.Request memory req = buildChainlinkRequest(
    ...
    function requestAssetsAndDefi(address _oracle, string memory _jobId)
        public
        onlyOwner
    {
        Chainlink.Request memory req = buildChainlinkRequest(
    ...
    function requestGovernance(address _oracle, string memory _jobId)
        public
        onlyOwner
    {
        Chainlink.Request memory req = buildChainlinkRequest(
    ...
    function fulfillBasicProfile(
        ...
        emit RequestBasicProfileFulfilled(
            _requestId,
            user,
            addr,
            ens,
    ...
    function fulfillSocialStatus(
        ...
        emit RequestSocialStatusFulfilled(
            _requestId,
            user,
            follower,
            following
        ...
        for (uint256 i; i < user.length; i++) {
            SocialStatusData[user[i]] = SocialStatus(
                follower[i],
                following[i],
                block.timestamp
            );
    ...
    function fulfillAssetsAndDefi(
        ...
        emit RequestassetsAndDefiFulfilled(
            _requestId,
            user,
            if_staking,
            nft,
    ...
    function fulfillGovernance(
        ...
        emit RequestGovernanceFulfilled(
            _requestId,
            user,
            snapshot,
            snapshot_link
        ...
        for (uint256 i; i < user.length; i++) {
            GovernanceData[user[i]] = Governance(
                snapshot[i],
                snapshot_link[i],
                block.timestamp
            );
